...
function setupSocketAPI(http) {
    ...
    gIo.on('connection', (socket) => {
        ...
        socket.on('task-updated', (data) => {
            ...
            logger.info(`New activity from socket [id: ${socket.id}], emitting to topic ${socket.myTopic}`);
            gIo.emit('task-updated', data);
        });
        socket.on('stay-updated', (stay) => {
            // console.log('stay update', stay)
            logger.info(`New stay update from socket [id: ${socket.id}], emitting to stayId ${socket.myTopic}`);
